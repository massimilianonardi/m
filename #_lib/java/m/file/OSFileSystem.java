...
public class OSFileSystem implements FileSystem, ConfigurableObject
{
  ...
  public void configure(Obj conf) throws Exception
  {
    ...
    m.Global.log.debug(root, rootPath.toString());
  ...
  protected Path path(String path) throws Exception
  {
    if(path == null)
    {
      m.Global.log.debug(rootPath.toString());
      return rootPath;
    }
    else if(separator == null)
    {
      m.Global.log.debug(rootPath.resolve(path).normalize().toString());
      return rootPath.resolve(path).normalize();
    }
    else
    {
      m.Global.log.debug(rootPath.resolve(path.replace("/", separator)).normalize().toString());
  ...
  {
    m.Global.log.debug(rootPath.toString(), rootPath.relativize(path(path).getParent()).toString(), path(path).toString());
