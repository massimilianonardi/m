...
bool SharedMemory::create(const char* key, long size)
{
  ...
  if(mmap == 0) exception_throw_type(ExceptionType::creation_failed)
  
  pb = MapViewOfFile(mmap, FILE_MAP_ALL_ACCESS, 0, 0, 0);
  if(pb == 0) exception_throw_type(ExceptionType::creation_failed)
#elif defined LINUX
  mmap = shmget((key_t) atoi(key), size, IPC_CREAT | IPC_EXCL | 00777);
  if(mmap == -1) exception_throw_type(ExceptionType::creation_failed)
  
  pb = shmat(mmap, 0, 0);
  if((long long) pb == -1) exception_throw_type(ExceptionType::creation_failed)
#else
#endif
  setPointers();
  if(pb)
...
bool SharedMemory::open(const char* key)
{
  ...
  if(mmap == 0) exception_throw_type(ExceptionType::open_failed)
  
  pb = MapViewOfFile(mmap, FILE_MAP_ALL_ACCESS, 0, 0, 0);
  if(pb == 0) exception_throw_type(ExceptionType::open_failed)
#elif defined LINUX
  mmap = shmget((key_t) atoi(key), 1000000, 0);
  if(mmap == -1) exception_throw_type(ExceptionType::open_failed)
  
  pb = shmat(mmap, 0, 0);
  if((long long) pb == -1) exception_throw_type(ExceptionType::open_failed)
#else
#endif
  setPointers();
  if(pb)
