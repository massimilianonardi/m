...
SERVICE_METHOD_DEFINITION(LoaderAdvanced, create)
{
  ...
  srv_params.links_ins(generateKey());
  srv_params.links_ins(generateKey());
  srv_params.links_ins(generateKey());
  if(0 < params.size()) srv_params.links_ins(params(0));
  
  // server. destroyed in this call after notify unlocks the lock.
  Service* ssrv = stream_dlib->create(Sequence((const char*) srv_params(0)).links_ins((Service*) this));
  
  // launch
  std::string cmd = "./process LoaderServer \"";
  cmd += std::string(srv_params.to_string()) + "\"";
  debug(cmd.c_str())
