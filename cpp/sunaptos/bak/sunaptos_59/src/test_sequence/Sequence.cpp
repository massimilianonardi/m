...
const char* SequenceTyped::to_text() const
{
  std::stringstream s;
  exception_try
//  s << "\n";
...
SequenceTyped& SequenceTyped::from_text(const char* t)
{
  resize(0);
//  sequences.resize(0);
  ...
  SequenceTyped start = s.substr(0, 2).c_str();
  if(start != (SequenceTyped) "$:")
  {
    exception_throw_type(Exception::creation_failed)
  }
  
  SequenceTyped end = s.substr(s.length() - 1).c_str();
  if(end != (SequenceTyped) ";")
  {
    exception_throw_type(Exception::creation_failed)
  }
  
  SequenceTyped type = s.substr(2, 2).c_str();
  
  SequenceTyped start_content = s.substr(4, 1).c_str();
  std::string subs = s.substr(5, s.length() - 6);
  if(start_content == (SequenceTyped) "<")
  {
    // search end content
    long n = s.find('>');
    ...
    SequenceTyped content = s.substr(5, n-5).c_str();
//    debug((const char*) content)
    if(type == (SequenceTyped) "i:")
    {
      *this = atoll(content);
    }
    else if(type == (SequenceTyped) "f:")
    {
      *this = atof(content);
    }
    else if(type == (SequenceTyped) "t:")
    {
      *this = content;
    }
    else if(type == (SequenceTyped) "u:")
    {
      *this = content;
    }
    else
...
//  debug(subs.c_str())
  ...
  {
    // check n+1 = [
    SequenceTyped start_sub = subs.substr(n+1, 1).c_str();
    if(start_sub != (SequenceTyped) "[")
    {
      exception_throw_type(Exception::creation_failed)
    }
