...
Buffer* SharedMemory::read(long size)
{
  Buffer* b = new Buffer();
  b->buffer = (char*) data + posr; // warning buffer must be copied outside!!!
  b->size = size;
  b->pos = size;
//  b->buffer = new char[n]; // lead to memory leaks
//  memcpy(b->buffer , data, *n);
  posr += size;
  *r = true;
//  std::cout << "\nSharedMemory::read pos " << b->pos << " buffer " << (char*) b->buffer;
...
{
//  std::cout << "\nSharedMemory::write pos " << buffer->pos << " buffer " << (char*) buffer->buffer;
  memcpy((char*) data + posw, buffer->buffer, buffer->pos);
  posw += buffer->pos;
  *w = true;
}
