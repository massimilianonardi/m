...
//      packet(0) = ri.header.hDevice; // todo put name instead, but use an efficient handle to name mapping
//      packet(1) = ri.header.dwType; // todo put my stander input type instead
//      packet(2) = value type
//      packet(3) = data
      
      free(pData);
      ihid = (service*) GetWindowLongPtr(hWnd, GWLP_USERDATA);
      ihid->write(sequence());
//      ihid->write(packet);
    break;
