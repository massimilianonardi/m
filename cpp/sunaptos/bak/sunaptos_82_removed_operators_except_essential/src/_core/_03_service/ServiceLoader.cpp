...
Service* ServiceLoader::try_create(SERVICE_METHOD_PARAMETERS)
{
  ...
  if((long long) srv == SERVICE_ERROR) exception_throw_type(ExceptionType::null_pointer)
  return srv;
}
...
void ServiceLoader::try_destroy(const Service* params)
{
  ...
  if(res == (Sequence) SERVICE_ERROR) exception_throw_type(ExceptionType::null_pointer)
}
