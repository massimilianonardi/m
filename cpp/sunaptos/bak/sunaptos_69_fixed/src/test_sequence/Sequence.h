...
protected:
  ...
  SequenceNode& copy(const SequenceNode& sequence_node){if(&sequence_node != this){*buffer = *sequence_node.buffer; sequence_links = sequence_node.sequence_links; basic_data_type = sequence_node.basic_data_type;}; return *this;}
  SequenceNode& move(SequenceNode& sequence_node){if(&sequence_node != this){buffer = std::move(sequence_node.buffer); sequence_links = std::move(sequence_node.sequence_links); basic_data_type = std::move(sequence_node.basic_data_type);}; return *this;}
  SequenceNode& link(SequenceNode& sequence_node){if(&sequence_node != this){buffer = sequence_node.buffer; sequence_links = &(sequence_node.sequence_links); basic_data_type = sequence_node.basic_data_type;}; return *this;}
...
// BASIC DATA TYPES CAST (basic_data_type, cast_pointer_get, cast_pointer_set) -
  ...
  SequenceNode& operator=(const void* basic_data_type_element){basic_data_type = BasicDataTypes::type_void_pointer; cast_pointer_set(&basic_data_type_element, sizeof(void*)); return *this;}
  SequenceNode& operator=(const char* basic_data_type_element){basic_data_type = BasicDataTypes::type_char; cast_pointer_set(basic_data_type_element, strlen(basic_data_type_element)); return *this;}
