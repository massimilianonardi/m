...
//  debug((char*) b2)
  ...
  b1 = "12345678";
  b2 = "test";
  debug("before    -> b1: " << (char*) b1 << " - b2: " << (char*) b2)
  debug("operation -> b2 = &b1; b1 = orimodi; -> link two sequences so that data changes are reflected in both")
  b2 = &b1;
  b1 = "orimodi";
  debug("after     -> b1: " << (char*) b1 << " - b2: " << (char*) b2)
  debug("expected  -> b1: orimodi - b2: orimodi")
  debug_line

  b1 = "linked";
  b2 = &b1;
  debug("before    -> b1: " << (char*) b1 << " - b2: " << (char*) b2)
  debug("operation -> b1 = new sequence(orimodi); -> tests trivial unlink with memory leak")
  b1 = new sequence("orimodi");
  debug("after     -> b1: " << (char*) b1 << " - b2: " << (char*) b2)
  debug("expected  -> b1: orimodi - b2: linked")
  debug_line

  b1 = "linked";
  b2 = &b1;
  debug("before    -> b1: " << (char*) b1 << " - b2: " << (char*) b2)
  debug("operation -> b1.unlink() = 12345678; -> tests unlink with proper method")
  b1.unlink() = "12345678";
  debug("after     -> b1: " << (char*) b1 << " - b2: " << (char*) b2)
  debug("expected  -> b1: 12345678 - b2: linked")
  debug_line
...
//  s1 << ((seq) "s0" << ((seq) "s1" << ((seq) "s2" << (seq) "s3")));
  ...
  s1.links_ins((sequence("s0").links_ins((sequence("s1").links_ins((sequence("s2").links_ins("s3")))))));
  debug(s1.to_string())
  s1.type_set(sequence_type::custom);
  debug((unsigned int) s1.type())
  s1.type_set(sequence_type::date);
  debug((unsigned int) s1.type())
  s1.type_set(sequence_type::number);
  debug((unsigned int) s1.type())
  s1.type_set(sequence_type::number_ieee754_double);
  debug((unsigned int) s1.type())
  s1.type_set(sequence_type::service);
  debug((unsigned int) s1.type())
