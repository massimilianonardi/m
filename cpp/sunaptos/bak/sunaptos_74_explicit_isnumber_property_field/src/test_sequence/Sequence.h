...
protected:
  ...
  SequenceNode& copy(const SequenceNode& sequence_node){if(&sequence_node != this){number = sequence_node.number; *buffer = *sequence_node.buffer; *object = *sequence_node.object; *sequence_links = *sequence_node.sequence_links;}; return *this;}
  SequenceNode& move(SequenceNode& sequence_node){if(&sequence_node != this){number = sequence_node.number; buffer = std::move(sequence_node.buffer); object = std::move(sequence_node.object); sequence_links = std::move(sequence_node.sequence_links);}; return *this;}
  SequenceNode& link(SequenceNode& sequence_node){if(&sequence_node != this){number = sequence_node.number; buffer = sequence_node.buffer; object = sequence_node.object; sequence_links = sequence_node.sequence_links;}; return *this;}
...
public:
  ...
  bool operator==(const SequenceNode& sequence_node) const {return (number == sequence_node.number) && (object == sequence_node.object) && (*buffer == *(sequence_node.buffer)) && (*sequence_links == *(sequence_node.sequence_links));}
...
// BASIC DATA TYPES CAST (basic_data_type, cast_pointer_get, cast_pointer_set) -
  ...
  SequenceNode& operator=(double basic_data_type_element){*this = std::move(SequenceNode()); number = true; cast_pointer_set(&basic_data_type_element, sizeof(double)); return *this;}
  ...
  bool is_number() const {return number;}
