...
Data* InterProcessCommunication::readparams()
{
  Data* p = new Data();
  setReadPos(nparams);
  p->read(this);
  std::cout << "\n[InterProcessCommunication::readparams] size = " << p->size().get();
...
Data* InterProcessCommunication::readres()
{
  Data* r = new Data();
  setReadPos(nres);
  r->read(this);
  std::cout << "\n[InterProcessCommunication::readres] size = " << r->size().get();
...
void InterProcessCommunication::writecmd(int cmd, Data* params)
{
  ...
  setWritePos(ncmdposted);
  write(&b);
  std::cout << "\n[InterProcessCommunication::writecmd] params size = " << params->size().get();
}

void InterProcessCommunication::writeres(Data* res)
{
  ...
  setWritePos(nansposted);
  write(&b);
  std::cout << "\n[InterProcessCommunication::writeres] res size = " << res->size().get();
}
