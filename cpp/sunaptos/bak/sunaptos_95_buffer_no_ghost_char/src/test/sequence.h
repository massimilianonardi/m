...
// BASIC DATA TYPES CAST -------------------------------------------------------
  ...
  sequence& operator=(const char* basic_data_type_element){*this = std::move(sequence()); set(basic_data_type_element, strlen(basic_data_type_element) + 1); return *this;}
  ...
  sequence_type& type(){if(size() <= 0) exception_throw_type(exception_type::index_out_of_boundaries); return (sequence_type&)(*this)[0];}
  const sequence_type& type() const {if(size() <= 0) exception_throw_type(exception_type::index_out_of_boundaries); return (sequence_type&)(*this)[0];}
//  sequence_type& type(){return (sequence_type&)(*this)[0];}
//  const sequence_type& type() const {return (sequence_type&)(*this)[0];}
