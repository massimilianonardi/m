...
function DateSelect()
{
  return DateSelect.Class.construct(this, arguments);
}

Class(DateSelect)
.inherit(ElementConfInputStore)
.property("conf", {locale: "en", format: "YYYY-MM-DD"})
...
//  return this.date.selectedDate.toISOString().split("T")[0];
//  return isNaN(this.date.selectedDate) ? "" : this.date.selectedDate.toISOString().split("T")[0];
  return value === null ? null : this.date.selectedDate.toISOString().split("T")[0];
//  return this.el.dateContainer.textContainer.textContent === "" ? null : this.date.selectedDate.toISOString().split("T")[0];
})
.setter("value", function(value)
{
//  this.input.value = value;
////  var d = value.split("-");
////  this.date.selectDate(parseInt(d[0]), parseInt(d[1]), parseInt(d[2]));
  
//  this.date.setDate(value);
  this.conf().defaultDate = value;
  if(value === null)
  {
    this.conf().defaultDate = "";
  }
  this.rebuild();
//  delete this.conf().defaultDate;
  
//  if(isNaN(this.date.selectedDate))
//  {
//    delete this.conf().defaultDate;
////    this.conf().defaultDate = null;
//    this.rebuild();
//  }
  if(this.date.el.dateContainer.textContainer.textContent === "")
  {
    return null;
  }
  else
...
//  return isNaN(this.date.selectedDate) ? "" : this.date.selectedDate.toISOString().split("T")[0];
//  console.log("setter", isNaN(this.date.selectedDate) ? null : this.date.selectedDate.toISOString().split("T")[0]);
//  return isNaN(this.date.selectedDate) ? null : this.date.selectedDate.toISOString().split("T")[0];
...
DateSelect.prototype.construct = function()
{
  this.rebuild();
};
...
DateSelect.prototype.rebuild = function()
{
  this.html("");
  ...
  var _this = this;
  var conf = this.conf();
  conf.container = this.input;
  conf.onSelect = function(date){conf.defaultDate = _this.date.selectedDate.toISOString().split("T")[0]; _this.value(conf.defaultDate);};
  this.date = new WinkelCalendar(conf);
  this.date.el.dateContainer.cancIcon.onclick = function(event)
  {
    _this.value("");
...
(function(window){
  'use strict';
	...
	// these are labels for the days of the week
	var cal_days_labels_en = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
	var cal_full_days_lables_en = ["Sunday","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"];

	// these are human-readable month name labels, in order
	var cal_months_labels_en = ['January', 'February', 'March', 'April',
	                     'May', 'June', 'July', 'August', 'September',
	                     'October', 'November', 'December'];
	var cal_months_labels_short_en = ['JAN', 'FEB', 'MAR', 'APR',
		                     'MAY', 'JUN', 'JUL', 'AUG', 'SEP',
		                     'OCT', 'NOV', 'DEC'];


	// these are labels for the days of the week
	var cal_days_labels_it = ['Do', 'Lu', 'Ma', 'Me', 'Gi', 'Ve', 'Sa'];
	var cal_full_days_lables_it = ["Domenica","Lunedì","Martedì","Mercoledì","Giovedì","Venerdì","Sabato"];

	// these are human-readable month name labels, in order
	var cal_months_labels_it = ['Gennaio', 'Febbraio', 'Marzo', 'Aprile',
	                     'Maggio', 'Giugno', 'Luglio', 'Agosto', 'Settembre',
	                     'Ottobre', 'Novembre', 'Dicembre'];
	var cal_months_labels_short_it = ['GEN', 'FEB', 'MAR', 'APR',
		                     'MAG', 'GIU', 'LUG', 'AGO', 'SET',
		                     'OTT', 'NOV', 'DIC'];
	...
	function WinkelCalendar(options){
		 this.defaults = {
          ...
          if(opts.locale === "it")
          {
            cal_days_labels = cal_days_labels_it;
            cal_full_days_lables = cal_full_days_lables_it;
            cal_months_labels = cal_months_labels_it;
            cal_months_labels_short = cal_months_labels_short_it;
          }
          else
          {
            cal_days_labels = cal_days_labels_en;
            cal_full_days_lables = cal_full_days_lables_en;
            cal_months_labels = cal_months_labels_en;
            cal_months_labels_short = cal_months_labels_short_en;
          }
		  this.init();
...
//		 this.el.dateContainer.calIcon.setAttribute('class','fa fa-calendar');
		 this.el.dateContainer.calIcon.setAttribute('class','wc-fa-calendar');
		 this.el.dateContainer.appendChild(this.el.dateContainer.calIcon);
		 this.el.dateContainer.cancIcon = document.createElement('i');
		 this.el.dateContainer.cancIcon.setAttribute('class','icon-cancel');
		 this.el.dateContainer.appendChild(this.el.dateContainer.cancIcon);
		 this.el.inputContainer.setAttribute('type','hidden');
		 this.el.inputContainer.setAttribute('class','wc-input');
		 this.el.dateContainer.setAttribute('class','wc-date-container input');
	...
	WinkelCalendar.prototype.setDateVal = function(){
            ...
            {
              this.selectedDate = new Date();
              this.el.dateContainer.textContainer.textContent = "";
              this.el.inputContainer.value = moment(this.selectedDate).format(this.options.format);
            }
	}
	...
	WinkelCalendar.prototype.createYearView = function(direction){			
			...
			this.el.calendarPopup.yearsView.prevYearList.classList.add('wc-fa-angle-left');
			...
			this.el.calendarPopup.yearsView.nextYearList.classList.add('wc-fa-angle-right');
	...
	WinkelCalendar.prototype.createHeaderView = function(){
		...
		// get first day of month
		  ...
		  if(self.options.bigBanner){
			  ...
			  self.el.bigBanner.prevMonthIcon.setAttribute('class', 'fa wc-fa-angle-left');
			  ...
			  self.el.bigBanner.nextMonthIcon.setAttribute('class', 'fa wc-fa-angle-right');
			 ...
			 // self.el.bigBanner.monthRow.appendChild(self.el.bigBanner.monthControls);
			  ...
			  self.el.bigBanner.prevYearIcon.setAttribute('class', 'fa wc-fa-angle-left');
			  ...
			  self.el.bigBanner.nextYearIcon.setAttribute('class', 'fa wc-fa-angle-right');
...
//			  console.log(self);
		 ...
		 // if(self.options.bigBanner){
		  ...
		  self.el.headerDetails.prevMonth.setAttribute('class', 'wc-prev fa wc-fa-angle-left');
		  ...
		  self.el.headerDetails.nextMonth.setAttribute('class', 'wc-next fa wc-fa-angle-right');
