...
#  echo "$k"
  ...
  echo "certificate:"
  cert="$(echo "$k" | openssl x509 -text -subject 2>/dev/null | awk '/BEGIN CERTIFICATE/,/END CERTIFICATE/ {print $0}')"
#  cert="$(echo "$cert" | awk '/BEGIN CERTIFICATE/,/END CERTIFICATE/ {print $0}')"
  echo "$cert"
done
