...
function Time()
{
  return Time.Class.construct(this, arguments);
}
...
.property("conf", {locale: "en", orientation: true})
...
Time.prototype.construct = function()
{
  this.rebuild();
};
...
Time.prototype.rebuild = function()
{
  this.html("");
...
//  var _this = this;
  var conf = this.conf();
  conf.el = this.el;
  conf.inputEl = this.input;
//  conf.toggleEl = this.toggle;
  conf.toggleEl = this.input;
  conf.type = "HOUR";
  conf.hourType = "24";
//  conf.onSelect = function(date){conf.defaultDate = _this.time.selectedDate.toISOString().split("T")[0];};
  this.time = new WindowDatePicker(conf);
