...
//      }
  ...
  public void credential(AuthorizatorSession session, String authentication, String id, String path, ObjInput in, ObjOutput out) throws Exception
  {
    ...
    Authenticator authn = (Authenticator) ((BaseAuthorizator) session.authorizator()).authenticators().get(authentication);
    AuthenticatorManager bauthn = (AuthenticatorManager) authn;
    if(authn.authenticate(id, new Obj(credential)) != false)
    {
      bauthn.setCredential(id, credentialNew);
      out.set(true);
    }
    else
    {
...
//    obj.set("authn-man", session.authenticate(authentication, null, null) == null ? true : false);
    Authenticator authn = (Authenticator) ((BaseAuthorizator) session.authorizator()).authenticators().get(authentication);
    obj.set("authn-man", authn.authenticate(null, null) == null ? true : false);
