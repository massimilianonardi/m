...
//  rl = dynamic_cast<RemoteLoader*>(dlibc->create(ipcc));
  ...
  rl = dynamic_cast<RemoteLoader*>(getServiceRemote("RemoteLoader", ipc_loader_key));
}
...
//  const char* c0 = params->gets(0)->get();
//  const char* c1 = params->gets(1)->get();
//  bool b2 = params->getn(2)->getb();
//  std::cout << "\n[ClientLoader::getInterface] ifn = " << c0 << " sn = " << c1 << " remote = " << b2;
...
void ClientLoader::makeServiceRemote(Object* obj, const char* name, const char* key)
{
  string srv_name = name + string("_srv.dlib");
  DLibSrv* dlibs = new DLibSrv(srv_name.c_str());
