...
public:
  ...
  char& operator[](const buffer_index& index){if(sz <= index < 0) exception_throw_type(exception_type::index_out_of_boundaries); return ((char*) get())[index];}
  const char& operator[](const buffer_index& index) const {if(sz <= index < 0) exception_throw_type(exception_type::index_out_of_boundaries); return ((char*) get())[index];}
  
  // custom optimized container methods
  const void* get() const {return (const void*) pb;}
  const void* get(const buffer_index& index) const {return (const void*)(((char*) pb) + index);}
