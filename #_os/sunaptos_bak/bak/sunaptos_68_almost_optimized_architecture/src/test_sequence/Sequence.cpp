...
std::string SequenceNode::text() const
{
  ...
  for(int i = 0; i < (long) operator()().size(); ++i)
  {
    s << "\n[sub-Sequence " << i << "]";
    s << (*this)(i).text();
  }
...
//  s << "\n";
  s << "$:";
  if(type() <= BasicDataTypes::type_unsigned_long_long_int)
  {
    if(size() == sizeof(long long)) s << "i:<" << (long long) *this << ">";
    else if(size() == sizeof(long)) s << "i:<" << (long) *this << ">";
    else s << "i:<u>";
  }
  else if(type() <= BasicDataTypes::type_double)
  {
    s << "f:<" << (double) *this << ">";
  }
  else if(type() <= BasicDataTypes::type_long_double)
  {
    s << "f:<" << (long double) *this << ">";
  }
  else if(type() == BasicDataTypes::type_char_pointer)
  {
    Buffer b;
...
//    s << "t:<" << et << ">";
  ...
  else if(type() == BasicDataTypes::type_void_pointer)
  {
