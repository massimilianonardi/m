...
public:
//  SequenceNode(): pb(0), sz(0), sequence_links(std::shared_ptr<std::vector<SequenceNode> >(new std::vector<SequenceNode>(), [this](std::vector<SequenceNode>* ptr){free(pb); pb = 0; delete ptr;})) {}
//  SequenceNode(): pb(0), sz(0), shptr(std::shared_ptr<void>(this, [](SequenceNode* psn){psn->destroy();})) {}
//  SequenceNode(): pb(0), sz(0), shptr(this, [](SequenceNode* psn){psn->destroy();}) {}
//  SequenceNode(): pb(0), sz(0), shptr(pb, [this](void* ptr){this->destroy();}) {resize(0);}
...
public:
  ...
  SequenceNode& del(const SequenceIndex& index);
...
// BASIC DATA TYPES CAST (get, set) --------------------------------------------
  ...
  SequenceNode& operator=(double basic_data_type_element){*this = std::move(SequenceNode()); set(&basic_data_type_element, sizeof(double)); (*this)[size()] = true; return *this;}
  ...
  bool is_number() const {if(size() == sizeof(double)) return (*this)[size()]; else return false;}
