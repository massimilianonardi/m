  ...
  switch(message)
  {
    case WM_INPUT:
    {
      ...
      else if(ri.header.dwType == RIM_TYPEKEYBOARD)
      {
        ...
        packet(3).links_resize(5);
        // todo unify scancode + left/right version flag (virtual key unification with l/r flag is not conceptually correct)
        // todo better fix for left/right flag and scancode remapping
//        packet(3)(0) = 0; // fixes windows bug not setting RI_KEY_E1, but only RI_KEY_E0 and in only in some cases
        double scancode = 0;
...
//        else if((ri.data.keyboard.Flags & RI_KEY_E1) == RI_KEY_E1)
//        {
//          packet(3)(0) = 0;
//        }
        packet(3)(0) = scancode + (double) ri.data.keyboard.MakeCode;
        
        packet(3)(1) = (double) ri.data.keyboard.MakeCode;
        packet(3)(2) = (double) ri.data.keyboard.VKey;
        packet(3)(3) = (double) ri.data.keyboard.Message;
        packet(3)(4).links_resize(6);
        packet(3)(4)(0) = (double) ri.data.keyboard.MakeCode;
        packet(3)(4)(1) = (double) ri.data.keyboard.Flags;
        packet(3)(4)(2) = (double) ri.data.keyboard.Reserved;
        packet(3)(4)(3) = (double) ri.data.keyboard.VKey;
        packet(3)(4)(4) = (double) ri.data.keyboard.Message;
        packet(3)(4)(5) = (double) ri.data.keyboard.ExtraInformation;
      }
