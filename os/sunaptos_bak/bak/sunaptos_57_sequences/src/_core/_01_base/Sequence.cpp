...
//  s << "\n";
  ...
  if(0 < (integer) size_sequences())
  {
    for(int i = 0; i < (integer) size_sequences(); ++i)
    {
      exception_try
      s << ":[" << get_sequence(i).to_text() << "]";
      exception_catch
...
//  debug(e.text())
  ...
  for(integer i = 0; i < (integer) size_sequences(); ++i)
  {
    res = res && (get_sequence(i) == e.get_sequence(i));
  }
  return res;
}
