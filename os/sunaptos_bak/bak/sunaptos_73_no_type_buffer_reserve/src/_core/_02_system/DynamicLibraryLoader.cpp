...
#else
#endif
  if(hinst == 0) exception_throw_type(ExceptionType::not_found)
}
...
{
  if(hinst == 0) exception_throw_type(ExceptionType::null_pointer)
  void* res = getFuncAddress(name);
  if(res == 0) exception_throw_type(ExceptionType::not_found)
  return res;
}
