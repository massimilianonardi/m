...
public:
  SequenceNode(): buffer(std::shared_ptr<Buffer>(new Buffer())), basic_data_type(BasicDataTypes::type_undefined), sequence_links(std::shared_ptr<std::vector<SequenceNode> >(new std::vector<SequenceNode>())) {}
  virtual ~SequenceNode(){}
  
  bool operator==(const SequenceNode& sequence_node) const {return (*buffer == *(sequence_node.buffer)) && (*sequence_links == *(sequence_node.sequence_links));}
...
//  SequenceNode& links_ins(SequenceNode&& sequence_node, const SequenceIndex& index){sequence_links->insert(sequence_links->begin() + index, sequence_node); return *this;}
//  SequenceNode& links_ins(SequenceNode* sequence_node, const SequenceIndex& index){sequence_links->insert(sequence_links->begin() + index, sequence_node); return *this;}
...
//  SequenceNode& sub(const SequenceIndex& index, const SequenceIndex& size){SequenceNode r; for(SequenceIndex i = 0; i < size; i++) r[i] = operator[](index + i); return r;}
//  SequenceNode& extract(const SequenceIndex& index, const SequenceIndex& size){SequenceNode r; for(SequenceIndex i = 0; i < size; i++) r[i] = operator[](index + i); buffer->del(index, size); return r;}
