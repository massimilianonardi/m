...
sequence& sequence::move(sequence& sequence_node)
{
  if(&sequence_node != this)
  {
    // two possible choices: move pointer, move data
    // the first inherits link relationships, the second keeps own link relationships
    // the first actually breaks the link system because of the subtle move operations performed by the compiler
    // move must be interpreted as a fast data copy, if linkage must be inherited, link operation must be explictly invoked
    
    // the following version unlinks this object from any other and import links from other object
//    destroy();
