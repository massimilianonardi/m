...
{
  if(type() == StandardDataTypes::sequence_dynamic)
  {
    // todo
  }
  else if(type() == StandardDataTypes::sequence_executable)
  {
    Service* srv = (*this)(0);
...
//  const char * hexmap = "0123456789ABCDEF";
  ...
  {
//    hex[i * 2] = hexmap[(source[i] & 0xF0) >> 4];
...
std::string SequenceNode::to_string() const
{
  ...
  else if(type() == StandardDataTypes::number_ieee754_double)
  {
    s << (double) *this;
  }
  else if((type() == StandardDataTypes::pointer) || (type() == StandardDataTypes::service))
  {
    s << hex((char*) pb, sz);
  }
  else
