...
//  s << "\n";
  s << "$:";
  if((*elements).type() <= BasicDataTypes::type_unsigned_long_long_int)
  {
    if((*elements).size() == sizeof(long long)) s << "i:<" << (long long) *this << ">";
    else if((*elements).size() == sizeof(long)) s << "i:<" << (long) *this << ">";
    else s << "i:<u>";
  }
  else if((*elements).type() <= BasicDataTypes::type_double)
  {
    s << "f:<" << (double) *this << ">";
  }
  else if((*elements).type() <= BasicDataTypes::type_long_double)
  {
    s << "f:<" << (long double) *this << ">";
  }
  else if((*elements).type() == BasicDataTypes::type_char_pointer)
  {
    BufferBasicDataTypes b;
    b.set((*elements).get(), (*elements).size());
    b.resize(b.size()+1);
    b[b.size()-1] = 0;
...
//    debug(et)
//    s << "t:<" << et << ">";
    s << "t:<" << (const char*) b.get() << ">";
  }
  else if((*elements).type() == BasicDataTypes::type_void_pointer)
  {
