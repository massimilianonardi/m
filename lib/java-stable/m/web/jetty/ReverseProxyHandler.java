...
public class ReverseProxyHandler extends AbstractHandler implements ConfigurableObject
{
  protected boolean secureRedirect;
  protected PrefixMap<String> services;
  ...
  public void construct(Obj args) throws Exception
  {
    ...
    construct(args.bool(Conf.SECURE), args.map(Conf.SERVICES));
  }
  ...
  public void construct(Boolean secureRedirect, Map services) throws Exception
  {
    destruct();
...
//    destruct();
  ...
  protected void execute(HttpServletRequest request, HttpServletResponse response) throws Exception
  {
    ...
    String proxyURL = services.prefix(command);
    String serviceKey = services.prefixKey(command);
    
    m.Global.log.debug(command, serviceKey, proxyURL, proxyURL.getClass().getName());
    ...
    reverseProxy.forwardTo(serviceKey, proxyURL, request, response);
  }
