...
public class DBReadWrite extends ConfigurableWrapper<Pool<Connection>> implements AuthorizedService
{
  ...
  public void execute(AuthorizationSession session, String command, ObjInput in, ObjOutput out) throws Exception
  {
    ...
    {
      read(session, command.substring(CMD_READ.length()), in, out);
    }
    else if(command.startsWith(CMD_WRITE))
    {
      write(session, command.substring(CMD_WRITE.length()), in, out);
    }
    else
    {
      throw new Exception();
  ...
  protected void read(AuthorizationSession session, String path, ObjInput in, ObjOutput out) throws Exception
  {
    SQLStatement sqlftm = new SQLStatement().sql(new SQLCommandHelper().select(path).paging(1).statement);
  ...
  protected void write(AuthorizationSession session, String path, ObjInput in, ObjOutput out) throws Exception
  {
    SQLStatement sqlftm = new SQLStatement().sql(new SQLCommandHelper().select(path).paging(1).statement);
    Map<String, Integer> fieldTypeMap = execute(sqlftm).fieldTypeMap;
