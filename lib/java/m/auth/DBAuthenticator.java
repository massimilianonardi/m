...
//    return sql.data.get("data").get(0).string(3);
  ...
  public List<String> list() throws Exception
  {
    ...
    m.Global.log.debug(sql);
    m.Global.log.debug(sql.data);
    m.Global.log.debug(sql.data.get("data"));
    
//    return sql.data.get("data").get(0).object(3) == null ? null : sql.data.get("data").get(0).object(3).toString();
    return sql.data.get("data").list();
  }
  ...
  {
//    throw new Exception();
    String insert = "insert into " + table + " set expiration='2000-01-01', " + columnCredential + "=md5('" + credential.replace("'", "''") + "'), " + columnID + "='" + id.replace("'", "''") + "';";
